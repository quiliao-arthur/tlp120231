...
FROM doce INNER JOIN doceingrediente ON doce.id = doceingrediente.iddoce INNER JOIN ingrediente ON doceingrediente.idingrediente = ingrediente.id 
...
FROM confeiteiro INNER JOIN cursoconfeiteiro ON confeiteiro.id = cursoconfeiteiro.idconfeiteiro INNER JOIN curso ON cursoconfeiteiro.idcurso = curso.id;
...
FROM curso INNER JOIN cursoinstituicao ON curso.id = cursoinstituicao.idcurso INNER JOIN instituicao ON cursoinstituicao.idinstituicao = instituicao.id 
INNER JOIN cursoconfeiteiro ON curso.id = cursoconfeiteiro.idcurso INNER JOIN confeiteiro ON cursoconfeiteiro.idconfeiteiro = confeiteiro.id
...
SELECT ingrediente.nome AS Ingrediente, doce.nome AS Doce_composto_pelo_Ingrediente, confeiteiro.nome AS Confeiteiro_criador_do_Doce
FROM ingrediente INNER JOIN doceingrediente ON ingrediente.id = doceingrediente.id.ingrediente INNER JOIN doce ON doceingrediente